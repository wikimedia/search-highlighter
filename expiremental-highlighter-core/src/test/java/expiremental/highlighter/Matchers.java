...
public class Matchers {
    ...
    public static <T> Matcher<Segment> extracted(SourceExtracter<T> extracter, Matcher<T> extractedMatcher) {
        return new ExtractedMatcher<T>(extracter, extractedMatcher);
    }
    public static <T> Matcher<HitEnum> hit(int position, SourceExtracter<T> extracter, Matcher<T> extractedMatcher) {
    ...
    private static class ExtractedMatcher<T> extends TypeSafeMatcher<Segment> {
        ...
        protected void describeMismatchSafely(Segment segment, Description description) {
            ...
            extractedMatcher.describeMismatch(extracter.extract(segment.startOffset(), segment.endOffset()), description);
        }

        @Override
        protected boolean matchesSafely(Segment segment) {
            return extractedMatcher.matches(extracter.extract(segment.startOffset(), segment.endOffset()));
        }    
    }
}
