 ...
 * @lucene.internal
 */
public class XBytesRefBuilder {

  private final BytesRef ref;

  /** Sole constructor. */
  public XBytesRefBuilder() {
    ref = new BytesRef();
  }
   ...
   * Append the provided bytes to this builder.
   */
  public void append(XBytesRefBuilder builder) {
    append(builder.get());
  }
  ...
  /**
   * Replace the content of this builder with the provided bytes. Equivalent to
   * calling {@link #clear()} and then {@link #append(XBytesRefBuilder)}.
   */
  public void copyBytes(XBytesRefBuilder builder) {
    clear();
    append(builder);
  }
  ...
  public void copyChars(CharSequence text, int off, int len) {
    grow(len * XUnicodeUtil.MAX_UTF8_BYTES_PER_CHAR);
    ref.length = XUnicodeUtil.UTF16toUTF8(text, off, len, ref.bytes);
  }
