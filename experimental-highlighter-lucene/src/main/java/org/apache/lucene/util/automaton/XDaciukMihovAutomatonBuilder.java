 ...
 */
final class XDaciukMihovAutomatonBuilder {
  /**
   ...
   */
  private static int convert(XAutomaton.Builder a, State s,
    ...
    int i = 0;
    int[] labels = s.labels;
    for (XDaciukMihovAutomatonBuilder.State target : s.states) {
   ...
   */
  public static XAutomaton build(Collection<BytesRef> input) {
    final XDaciukMihovAutomatonBuilder builder = new XDaciukMihovAutomatonBuilder();
    
    char[] chars = new char[0];
    CharsRef ref = new CharsRef();
    for (BytesRef b : input) {
      chars = ArrayUtil.grow(chars, b.length);
      final int len = XUnicodeUtil.UTF8toUTF16(b, chars);
      ref.chars = chars;
      ref.length = len;
      builder.add(ref);
    }
    
    XAutomaton.Builder a = new XAutomaton.Builder();
    convert(a,
        builder.complete(), 
