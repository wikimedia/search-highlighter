...
public class TestCompiledAutomaton extends LuceneTestCase {
  ...
  private XCompiledAutomaton build(int maxDeterminizedStates, String... strings) {
    final List<BytesRef> terms = new ArrayList<>();
    for(String s : strings) {
      terms.add(new BytesRef(s));
    }
    Collections.sort(terms);
    final XAutomaton a = XDaciukMihovAutomatonBuilder.build(terms);
    return new XCompiledAutomaton(a, true, false, maxDeterminizedStates);
  }

  private void testFloor(XCompiledAutomaton c, String input, String expected) {
    final BytesRef b = new BytesRef(input);
    final BytesRef result = c.floor(b, new XBytesRefBuilder());
    if (expected == null) {
      assertNull(result);
    } else {
  ...
  private void testTerms(int maxDeterminizedStates, String[] terms) throws Exception {
    final XCompiledAutomaton c = build(maxDeterminizedStates, terms);
...
//        System.out.println("  expected=" + expected);
  ...
  public void testBasic() throws Exception {
    XCompiledAutomaton c = build(XOperations.DEFAULT_MAX_DETERMINIZED_STATES,
      "fob", "foo", "goo");
    testFloor(c, "goo", "goo");
    testFloor(c, "ga", "foo");
    testFloor(c, "g", "foo");
